...
export function formatError(error: any) {
  if (error?.name === 'ZodError') {
    // Handle Zod error (supports both .issues and .errors shapes and flatten())
    try {
      if (typeof error.flatten === 'function') {
        ...
        const formMessages = (flat?.formErrors ?? []).filter(Boolean) as string[];
      ...
      const messages = (issues as Array<{ message?: string }>)
        .map((i) => i?.message)
        .filter(Boolean) as string[];
